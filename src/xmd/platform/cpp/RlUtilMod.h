...
RLUTIL_INLINE int xpos() {
#if defined(_WIN32) && !defined(RLUTIL_USE_ANSI)
	CONSOLE_SCREEN_BUFFER_INFO csbi;
	GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi);
	return (csbi.dwCursorPosition.X + 1); // Windows uses 0-based coordinates
#else // _WIN32 || USE_ANSI
	char buf[8];
...
RLUTIL_INLINE int ypos() {
#if defined(_WIN32) && !defined(RLUTIL_USE_ANSI)
	CONSOLE_SCREEN_BUFFER_INFO csbi;
	GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi);
	return (csbi.dwCursorPosition.Y + 1); // Windows uses 0-based coordinates
#else // _WIN32 || USE_ANSI
	char buf[8];
	struct termios save,raw;
	tcgetattr(0,&save);
